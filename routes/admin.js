...
router.post("/pending-hours", adminAuth, async (req, res) => {
  ...
  try {
    ...
    loggerFunction(
      "debug",
      `${route} - Sample Record: ${JSON.stringify(
        pendingHours[0] || {},
        null,
        2
      )}`
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (entry.status !== "pending") {
        loggerFunction(
          "warn",
          `${route} - Cannot update. Status=${entry.status}`
        );
      ...
      loggerFunction(
        "info",
        `${route} - Entry updated successfully. id=${entry._id}`
      );
...
//     loggerFunction(
//       "debug",
//       `${route} - Id=${req.params.id}, Incoming request body=${JSON.stringify(
//         req.body
//       )}`
//     );
...
//       loggerFunction(
//         "warn",
//         `${route} - Invalid status provided. Id=${req.params.id} status=${status}`
//       );
...
//       loggerFunction(
//         "warn",
//         `${route} - Hours entry not found. Id=${req.params.id}`
//       );
//       return res.status(404).json({ message: "Hours entry not found" });
//     }
//     loggerFunction(
//       "debug",
//       `${route} - Hours entry found. Id=${req.params.id} Data=${JSON.stringify(
//         hoursEntry
//       )}`
//     );
...
//       loggerFunction(
//         "debug",
//         `${route} - Rejection reason set. Id=${req.params.id}`
//       );
//     } else {
...
//     loggerFunction(
//       "info",
//       `${route} - Hours entry updated and saved. Id=${req.params.id} newStatus=${status}`
//     );
...
//       loggerFunction(
//         "warn",
//         `${route} - Volunteer user not found for entry. volunteerId=${hoursEntry.volunteerId}`
//       );
//     }
...
//           newTier &&
//           newTier !== "None" &&
//           !volunteer.badges.includes(newTier)
//         ) {
...
//       const displayName =
//         firstName || lastName ? `${firstName} ${lastName}`.trim() : "Volunteer";

//       const entryInfoHtml = `
//     <p><strong>Activity:</strong> ${hoursEntry.activityName}</p>
//     <p><strong>Date of service:</strong> ${formatDate(
//       hoursEntry.serviceDate
//     )}</p>
...
//           loggerFunction(
//             "debug",
//             `${route} - Sending approval email to ${volunteer.email}`
//           );
...
//           loggerFunction(
//             "error",
//             `${route} - Approval email failed: ${err.message}`
//           );
//         }
//       }
...
//           loggerFunction(
//             "error",
//             `${route} - Rejection email failed: ${err.message}`
//           );
//         }
//       }
...
//           loggerFunction(
//             "warn",
//             `${route} - No tier message found for tier: ${newTier}`
//           );
//         } else {
//           const bodyHtml = `
...
//             loggerFunction(
//               "error",
//               `${route} - Attachment error: ${err.message}`
//             );
//           }
...
//             loggerFunction(
//               "debug",
//               `${route} - Sending tier upgrade email for tier=${newTier}`
//             );
...
//             loggerFunction(
//               "error",
//               `${route} - Tier email failed: ${err.message}`
//             );
//           }
//         }
//       }
...
//     loggerFunction(
//       "info",
//       `${route} - Response sent successfully. Id=${req.params.id}`
//     );
//     loggerFunction(
//       "debug",
//       `${route} - Response body sample. Id=${
//         req.params.id
//       } Data=${JSON.stringify(hoursEntry)} status=${hoursEntry.status}`
//     );
...
router.put("/review-hours/:id", adminAuth, async (req, res) => {
  ...
  try {
    loggerFunction("info", `${route} - Started. Id=${req.params.id}`);
    ...
    if (status === "rejected") {
      hoursEntry.rejectionReason = rejectionReason || "";
      await hoursEntry.save();
      return res.json({ message: "Hours rejected", entry: hoursEntry });
    }
    ...
    // ------------------ Tier Configuration ------------------
    const TIERS = [
      { name: "Kindness Ambassador", minHours: 50 },
      { name: "Change Catalyst", minHours: 100 },
      { name: "Service Champion", minHours: 150 },
      { name: "Legacy Leader", minHours: 250 }
    ];

    const previousTotalHours = volunteer.totalHours || 0;
    const addedHours = hoursEntry.hours || 0;

    volunteer.totalHours = previousTotalHours + addedHours;

    // ------------------ Update This Year Hours ------------------
    const currentYear = new Date().getFullYear();
    const serviceYear = new Date(hoursEntry.serviceDate).getFullYear();

    if (serviceYear === currentYear) {
      volunteer.thisYearHours = (volunteer.thisYearHours || 0) + addedHours;
    }

    // ------------------ Badge & Tier Processing ------------------
    let newlyEarnedTiers = [];

    for (const tier of TIERS) {
      const crossed = previousTotalHours < tier.minHours && volunteer.totalHours >= tier.minHours;

      if (crossed && !volunteer.badges.includes(tier.name)) {
        volunteer.badges.push(tier.name);
        newlyEarnedTiers.push(tier.name);
      }
    }

    if (newlyEarnedTiers.length > 0) {
      volunteer.tier = newlyEarnedTiers[newlyEarnedTiers.length - 1];
      volunteer.lastAchievedTier = volunteer.tier;
      volunteer.lastTierUpdatedAt = new Date();
    }

    await volunteer.save();

    loggerFunction("info", `${route} - Volunteer updated. NewBadges=${JSON.stringify(newlyEarnedTiers)}`);

    // ------------------ EMAILS ------------------
    const displayName = `${volunteer.profile?.firstName || ""} ${volunteer.profile?.lastName || ""}`;
  ...
  `;
    ...
    // -------- Tier Emails (ONE BY ONE ASCENDING) --------
    for (const tierName of newlyEarnedTiers) {
      ...
      const tierHtml = renderEmailTemplate({
        ...
        body: `
          <p>🎉 <strong>Congratulations!</strong></p>
          <p>${tierInfo.message}</p>
          <p>You earned the <strong>${tierName}</strong> badge.</p>
        `
      });

      const attachments = [];
      try {
        attachments.push(getAttachment(`NEST4US Recognition Tiers Social Media Toolkit_${tierName}.pdf`));
        attachments.push(getAttachment(`${tierName}.png`));
      } catch (err) {
    ...
    loggerFunction("error", `${route} - ${error.stack || error.message}`);
    res.status(500).json({ message: "Server error", error: error.message });
  }
});
...
router.get("/volunteers", adminAuth, async (req, res) => {
  ...
  try {
    ...
    if (!volunteers.length) {
      ...
      loggerFunction(
        "info",
        `${route} - Retrieved ${volunteers.length} volunteer record(s).`
      );
      loggerFunction(
        "debug",
        `${route} - Sample volunteer record: ${JSON.stringify(
          volunteers,
          null,
          2
        )}`
...
router.get("/stats", adminAuth, async (req, res) => {
  ...
  try {
    ...
    loggerFunction(
      "info",
      `${route} - pendingSubmissions=${pendingSubmissions}`
    );
      ...
      const summary = await VolunteerHours.aggregate([
        ...
        },
        {
          $project: {
            ...
            totalValue: {
              $round: [
                { $multiply: ["$totalHours", VOLUNTEER_HOURLY_RATE] },
                2,
              ],
            },
          },
    ...
    if (
      ...
      if (volunteerName && volunteerName.trim() !== "") {
        ...
        const nameConditions = words.map((word) => {
          ...
          return {
            $or: [
              { "volunteer.profile.firstName": regex },
              { "volunteer.profile.lastName": regex },
            ],
          };
        });
      ...
      pipeline.push(
        { $sort: { serviceDate: -1 } },
        {
          $project: {
            ...
            "Date of Service": {
              $dateToString: { format: "%m-%d-%Y", date: "$serviceDate" },
            },
...
router.get("/summary", adminAuth, async (req, res) => {
  ...
  try {
    ...
    loggerFunction(
      "debug",
      `${route} - Summary data: ${JSON.stringify(summary, null, 2)}`
    );
...
router.get("/tiers", adminAuth, async (req, res) => {
  ...
  try {
    loggerFunction(
      "info",
      `${route} - API execution started. adminId=${req.user._id}`
    );
    ...
    loggerFunction(
      "debug",
      `${route} - counts=${JSON.stringify(tierCounts, null, 2)}`
    );
...
router.post("/tiers", adminAuth, async (req, res) => {
  ...
  try {
    ...
    loggerFunction(
      "info",
      `${route} - API execution started. userId=${req.user._id} tier=${tierName}`
    );
...
router.get("/analytics/age-distribution", adminAuth, async (req, res) => {
  ...
  try {
    ...
    loggerFunction(
      "debug",
      `${route} - Found ${volunteers.length} volunteers with dateOfBirth`
    );
    ...
    loggerFunction(
      "info",
      `${route} - Age distribution calculated successfully`
    );
    loggerFunction(
      "debug",
      `${route} - Distribution: ${JSON.stringify(ageGroups)}`
    );
...
router.get("/analytics/hours-by-category", adminAuth, async (req, res) => {
  ...
  try {
    ...
    const approvedHours = await VolunteerHours.find({
      status: "approved",
    }).select("serviceType hours");

    loggerFunction(
      "debug",
      `${route} - Found ${approvedHours.length} approved hour entries`
    );
    ...
    const labelMapping = {
      ...
      "NEST4US Impact Internship": "Impact Internships",
      Other: "Other",
    };
    ...
    loggerFunction(
      "debug",
      `${route} - Normalized category stats: ${JSON.stringify(categoryStats)}`
    );
    loggerFunction(
      "info",
      `${route} - Total hours across all categories: ${totalHours}`
    );
    ...
    loggerFunction(
      "debug",
      `${route} - Final response: ${JSON.stringify(response)}`
    );
...
router.get("/users/search", adminAuth, async (req, res) => {
  try {
    ...
    const users = await User.find({ $and: searchConditions })
      ...
      .select(
        "email profile.firstName profile.lastName profile.profilePicture"
      );
